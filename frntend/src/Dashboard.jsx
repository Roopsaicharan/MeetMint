    ...
    const [projects, setProjects] = useState([
        {
            ...
            notes: '',
            summary: {
                summary: 'The MeetMint team finalized the Sprint 1 requirements and successfully implemented the Glassmorphism theme.',
                decisions: ['Frontend will use Glassmorphism for all new modules', 'Backend Go server to handle dynamic status updates'],
                action_items: [
        ...
        { id: 2, name: 'Mobile Prototype', status: 'In Review', dueDate: '2026-02-18', daysLeft: 3, members: 3, progress: 92, notes: '', summary: null }
    ])
                        ...
                        <div className="header-content">
                            <div className="logo-section">
                                <div className="brand-header">
                                    ...
                                    <h1 className="dash-logo" onClick={() => setView('home')}>MeetMint</h1>
                                </div>
                        ...
                        <div className="header-content">
                            <div className="logo-section">
                                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                    <button onClick={() => setView('home')} className="glass-btn-secondary" style={{ padding: '5px 10px' }}>← Back</button>
                                    <div className="brand-header">
                                        <MMLogo className="header-logo" />
                                        <h1 className="dash-logo" onClick={() => setView('home')} style={{ margin: 0 }}>MeetMint</h1>
                                    </div>
                                </div>
                                <p className="dash-subtitle">Project: <strong>{activeProject.name}</strong> ({activeProject.daysLeft} days left)</p>
                            </div>
                            <div className="dash-user">
                                <span className="dash-email">{user?.email}</span>
                                ...
                                {(summary || activeProject.summary) ? (
                                    <section className="glass-section glass-result fade-in" style={{ height: '100%' }}>
                                        <div className="section-header">
                                            ...
                                            <span className="glass-badge" style={{ background: 'rgba(34, 197, 94, 0.2)', color: '#4ade80' }}>Complete</span>
                                        </div>
                                        <p className="glass-summary-text">{summary ? summary.summary : activeProject.summary.summary}</p>

                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem', marginTop: '1.5rem' }}>
                                            <div>
                                                <h4>Key Decisions</h4>
                                                <ul className="glass-list">
                                                    {(summary ? summary.decisions : activeProject.summary.decisions).map((d, i) => <li key={i}>{d}</li>)}
                                                </ul>
                                            </div>
                                    ...
                                    <div className="glass-section" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: '400px', opacity: 0.6 }}>
                ...
                <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '3rem 2rem' }}>
                    ...
                    <div className="projects-grid">
                        {filteredProjects.map(project => (
                            <div
                                key={project.id}
                                className={`project-card ${selectionMode && selectedProjects.has(project.id) ? 'selected' : ''}`}
                                onClick={() => selectionMode ? toggleProjectSelection(project.id) : openProject(project)}
                                style={{
                                    border: selectionMode && selectedProjects.has(project.id) ? '2px solid #60a5fa' : '',
                                    transform: selectionMode && selectedProjects.has(project.id) ? 'translateY(-5px)' : ''
                                }}
                            >
                                {selectionMode && (
                                ...
                                <span className={`status-badge ${project.status === 'Active' ? 'active' : 'review'}`}>
                                    {project.status}
                                </span>
