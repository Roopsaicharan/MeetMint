...
import whisper
# Using 'base' model for extremely fast prototyping. Switch back to 'medium' for production accuracy.
...
def transcribe():
    ...
    try:
        ...
        if clip.audio is None:
            clip.close()
            print("❌ Error: The uploaded video has no audio track.")
            return jsonify({"error": "No audio track found in video."}), 400
            
        print("🎵 Exporting audio track to WAV format...")
